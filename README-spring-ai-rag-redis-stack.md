...
- 현재 인덱싱 가능한 문서의 종류는 마크다운, PDF, HWPX 파일로 구성되어 있다.
- `application.yml` 의 `spring.ai.document.path`, `spring.ai.document.pdf-path`, `spring.ai.document.hwpx-path` 에서 확인 가능하다.

### HWPX 문서 활성화

`application.yml`의 `spring.ai.document.hwpx-path` 항목이 기본적으로 주석 처리되어 있다. HWPX 파일을 인덱싱하려면 해당 주석을 해제하고 경로를 설정한다.
...
경로를 설정하지 않으면 HWPX 리더는 건너뛰며, 다른 형식의 문서 처리에는 영향을 주지 않는다.
