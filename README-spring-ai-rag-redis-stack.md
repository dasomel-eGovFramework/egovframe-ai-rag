...
- 현재 인덱싱 가능한 문서의 종류는 마크다운 파일, PDF 파일, HWP 파일, Excel(.xlsx) 파일이다.
- `application.yml` 의 `spring.ai.document.path`, `spring.ai.document.pdf-path`, `spring.ai.document.hwp-path`, `spring.ai.document.xlsx-path` 에서 확인 가능하다. HWP/Excel 경로 속성이 없으면 해당 처리를 건너뛴다.
